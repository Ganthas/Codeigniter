...
class DataMapper implements IteratorAggregate
{
	...
	public function DataMapper($id = null)
	{
		...
		if ($is_dmz) {
			...
			foreach (DataMapper::$_dmz_config_defaults as $config_key => $config_value) {
				if (is_array(DataMapper::$config) && !array_key_exists($config_key, DataMapper::$config)) {
		...
		if (!isset(DataMapper::$common[$common_key])) {
			...
			if (!$loaded_from_cache) {
				...
				foreach ($this->has_one as $related => $rel_props) {
					$field = $related . '_id';
					if (in_array($field, $this->fields) && (!isset($this->validation[$field]) || // does not have a validation key or...
				...
				foreach (array(
					...
					'has_one', 'has_many', '_field_tracking'
				) as $item) {
	...
	public function __get($name)
	{
		...
		if ($has_many || $has_one) {
			...
			if ($this->exists() && ($has_many && ($this->auto_populate_has_many || $this->has_many[$name]['auto_populate'])) || ($has_one && ($this->auto_populate_has_one || $this->has_one[$name]['auto_populate']))) {
				$this->{$name}->get();
			}
	...
	public function get_clone($force_db = false)
	{
		...
		if ($force_db || (($this->db_params !== false) && isset($this->db))) {
	...
	private function _process_function_arg($arg, $is_formula = false)
	{
		...
		if (is_string($arg)) {
			if (($is_formula && in_array($arg, $operators)) ||
				$arg == '*' || ($arg[0] == "'" && $arg[strlen($arg) - 1] == "'") || ($arg[0] == "[" && $arg[strlen($arg) - 1] == "]")) {
	...
	private function _subquery($query, $args)
	{
		...
		if ($query == 'select') {
			...
			if (is_object($args[0]) || (is_string($args[0]) && !isset($args[1]))) {
	...
	protected function _handle_default_order_by()
	{
		...
		if (empty($this->db->ar_orderby) && (empty($this->db->ar_select) ||
	...
	protected function _dmz_assign_libraries()
	{
		...
		if ($CI || $CI = &get_instance()) {
			...
			// to the config
			// self::get_config_object() = &$CI->config;
